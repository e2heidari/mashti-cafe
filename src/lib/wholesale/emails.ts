...
type AdminFinalOrderCopyEmailInput = {
  orderNumber: string;
  customer: WholesaleOrderCustomer;
...
export function buildAdminFinalOrderCopyEmail({
  orderNumber,
  customer,
  customerEmail,
  items,
  ...
  const text = `
Final order sent to customer (internal copy)
...
Finalized Total: $${finalizedTotalAmount.toFixed(2)}${sellerNoteText}

This is an internal copy for your records. The customer received the final order email separately.
`.trim();

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto;">
      <h2 style="color: #e80812; margin-bottom: 8px;">Final order sent to customer</h2>
      <p style="color: #666666; font-size: 13px; margin: 0 0 20px;">Internal copy — not sent to the customer.</p>

      <div style="background-color: #e80812; color: white; padding: 16px 20px; border-radius: 8px; margin-bottom: 20px;">
        <p style="margin: 0 0 8px;"><strong>Order Number:</strong> ${safeOrderNumber}</p>
        <p style="margin: 0;"><strong>Sent To:</strong> ${safeCustomerEmail}</p>
      </div>
      ...
      <div style="margin-bottom: 20px;">
        <h3 style="color: #333; margin: 0 0 12px;">Finalized Items</h3>
        <table
          role="presentation"
          cellpadding="0"
          cellspacing="0"
        ...
        >
          ...
          <tfoot>
            <tr style="background-color: #fff5f5;">
              <td colspan="6" align="right" style="padding: 14px 10px; font-size: 15px; font-weight: 700; color: #333333; border-top: 2px solid #e80812;">Finalized Total</td>
              <td align="right" style="padding: 14px 10px; font-size: 18px; font-weight: 700; color: #e80812; border-top: 2px solid #e80812; white-space: nowrap;">$${finalizedTotalAmount.toFixed(2)}</td>
            </tr>
          </tfoot>
        </table>
      ...
      ${
        safeSellerNote
          ? `
      <div style="background-color: #f9f9f9; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h3 style="color: #333; margin-top: 0;">Seller Note</h3>
        <p style="margin: 0; color: #333333; font-size: 14px; line-height: 1.5; white-space: pre-wrap;">${safeSellerNote}</p>
      </div>
      `
          : ""
  ...
  return {
    subject: `${subjectPrefix}: Final order sent to customer — ${customer.businessName} (${orderNumber})`,
    text,
    html,
  };
}
